...
% Importando os dados
% X = csvread('~/Dropbox/Dados/VETOR_IMAGENS/VECTOR_IMAGE_Ordenado_INTC_Baixo Erro_GENERO.csv');
...
X_label = [ones(230,1); 2*ones(230,1)]; %Classifica��o em 4 grupos
nFold = 10; % N�mero de K-fold

% Ordena os dados para o k-fold, separando o que ser� usado para Teste / Treino
indicesRuins = crossvalind('Kfold', X_label(1:230,:), nFold);
indicesBons = crossvalind('Kfold', X_label(231:end,:), nFold);
...
[Pmlda, Kmlda, Vmlda] = ctmlda(Ytrain, 2, ...
    ...
    , length(find(X_label(xtrain)==2))], 1);
...
% Plotando dados do Treinamento
    subplot(nFold,1,i)
    p1=plot(Ztrain(find(TRlabel==1)),0,'ro', 'MarkerSize', 10, 'LineWidth', 2); hold on; % Ruim
    ...
    p4=plot(Ztrain(find(TRlabel==2)),0,'kx', 'MarkerSize', 10, 'LineWidth', 2); hold on; % BOM
...
for idx = find(TSlabel==2)
    p6=plot(Ztest(idx),0,'ko', 'MarkerSize', 15, 'LineWidth', 2); hold on; % M�dio
end

for idx = find(TRlabel==1)
mRuins = mean(Ztrain(idx));
p7 = plot(mRuins,0,'r*', 'MarkerSize', 30, 'LineWidth', 1); hold on;
end
for idx = find(TRlabel==2)
mBons = mean(Ztrain(idx));
p8 = plot(mBons,0,'kx', 'MarkerSize', 30, 'LineWidth', 1); hold on;
end
