...
setwd('C:/Users/Trabalho/Desktop/Adicione1') #Configura??o do diret?rio (Desk Home).

# Comando para ler a base e dados e atribu?-la a uma vari?vel
pupil <- read.csv("Dados59.tsv", header = TRUE, sep = "\t")  #variavel <- read.tsv("aqruivo.csv", header = TRUE(usa primeira linha como t?tulo), sep = "\t" (separador \t = tab)) 
# ----------------------------------------------------------------------------------------------------
# ETAPA 2:FILTRO E ORGANIZA??O DOS DADOS
# Nesta etapa s?o removidos todos os dados desnecess?rios, como telas de instru??es e intervalos entre tarefas.
# Ap?s a aplica??o dos filtros, os dados s?o organizados para faciltiar as pr?cimas etapas.
...
pupil <- pupil[pupil$MediaName!="Inst2.jpg", ] #Filtra Slide de instru??o (Inst2.jpg)
pupil <- pupil[pupil$MediaName!="Slide2.JPG", ] #Filtra Slide de instru??o (Slide2.jpg)
...
pupil <- pupil[pupil$MediaName!="Slide19.JPG", ] #Filtra Slide de instru??o (Slide19.JPG)
pupil <- pupil[pupil$MediaName!="Slide20.JPG", ] #Filtra Slide de instru??o (Slide20.JPG)
pupil <- pupil[pupil$MediaName!="Slide31.JPG", ] #Filtra Slide de instru??o (Slide31.JPG)
pupil <- pupil[pupil$MediaName!="FIM.jpg", ] #Filtra Slide de instru??o (FIM.jpg)

# Organiza??o dos dados
pupil$X <- NULL #Remove a coluna X (coluna em branco inserida pelo arquivo TSV)
pupil$Participante <- pupil$?..ParticipantName #Copia a coluna ?..ParticipantName para Participante
...
pupil$?..ParticipantName <- NULL #N?o usa mais, remove para diminuir o processamento
pupil$MediaName <- NULL #N?o usa mais, remove para diminuir o processamento
pupild <- pupil # Copia a base para preservar di?metro das pupilas usado na an?lise do PCA
...
# Fun??o pra gerar lista de vari?ncia por participante/slide.
...
fvar = function(participante,slide){
  ...
  for (j in c(participante)){
    ...
    for(i in c(slide)){
      ...
      lista[r,c] <- var(x2$Diametro) # Calcula a Vari?ncia de cada participante por Slide e monta uma lista.
      c<-c+1
    }
    r<-r+1
  }
  return(lista) #Retorna lista com coeficientes de vari?ncia de cada participante.
}

# Fun??o para gerar lista de vari?ncia por tarefa (Tarefa =  R1 + Q1)
ftask = function(participante,pupil){
  ...
  for (j in c(participante)){
    ...
    # Calcula a Vari?ncia de cada participante por tarefa e monta uma lista.
  ...
  return(var2tarefa) #Retorna lista com coeficientes de vari?ncia de cada participante.
}
...
# Fun??o para agrupar os coeficientes de vari?ncia
# Coluna 1: M?dia dos coeficientes de variancia das 5 tarefas de Adicione 1
# Coluna 2: M?dia dos coeficientes de variancia das 5 tarefas de Adicione 3
# Coluna 3: M?dia dos coeficientes de variancia das 10 tarefas (Adicione 1 + Adcione 3)
fmean = function(lista){
  ...
  return(lista2) #Retorna lista com coeficientes de vari?ncia de cada participante.
}
...
# Agrupamento das vari?veis da tarefa adcione 1.
# Coluna 1: Coeficientes da etapa de memoriza??o;
# Coluna 2: Coeficientes da etapa de reprodu??o e transforma??;
...
# Agrupamento das vari?veis da tarefa adcione 3.
# Coluna 1: Coeficientes da etapa de memoriza??o;
# Coluna 2: Coeficientes da etapa de reprodu??o e transforma??;
# Coluna 3: Coeficientes globais.
...
# Vari?ncia por grupos.  ----------------- REMOVI P06, P23, P24 E P36 - OUTLIERS
...
# Prepara??o para plotar compara??o entre grupos
...
# Agrupamento das vari?veis os grupos.
...
#PLOTS
#1
pdf("CBA 1 - Vari?ncia por cart?o.pdf",width = 5, height = 3.5)
...
boxplot(x = as.list(as.data.frame(varianciaporcartao)),ylim=c(0,.22),xaxt = "n",
        main="Fase A", 
        xlab="Cart?es", ylab="Vari?ncia do di?metro da pupila (cm)")
...
#2
pdf("CBA 4 - Vari?ncia por tarefa agrupado.pdf",width = 5, height = 3.5)
...
boxplot(x = as.list(as.data.frame(add1)),ylim=c(0,.226),xaxt = "n",
        ...
        ylab="Vari?ncia do di?metro da pupila (mm)")
...
#3
pdf("CBA 9c - Compara??o de grupos Global.pdf",width = 5, height = 3.5)
par(mfrow=c(1,2),mar=c(2,4,1,1),bty="l",cex.axis=0.6,cex.lab=1)
boxplot(x = as.list(as.data.frame(acertos2)),ylim=c(0,.226),xaxt = "n",
        main="Proficientes",ylab="Vari?ncia do di?metro da pupila (cm)")
axis(1,at=1:2,labels=c("Fase A", "Fase B"))
boxplot(x = as.list(as.data.frame(erros2)),ylim=c(0,.226),xaxt = "n",
        main="N?o-proficientes", ylab="Vari?ncia do di?metro da pupila (cm)")
axis(1,at=1:2,labels=c("Fase A", "Fase B"))
dev.off()

#4
pdf("CBA 10 - Dispers?o dos dados de treinamento.pdf",width = 5, height = 3.5)
par(mfrow=c(1,1),mar=c(2,4,1,1),bty="l",cex.axis=0.8,cex.lab=1)
boxplot(x = as.list(as.data.frame(classedisper)),xaxt = "n",
        ylab="Caracter?sticas mais discriminantes")
axis(1,at=1:2,labels=c("Proficientes", "N?o-proficientes"))
dev.off()
